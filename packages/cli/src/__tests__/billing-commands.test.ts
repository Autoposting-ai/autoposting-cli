/**
 * Integration tests for billing commands.
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ap-billing-cmd-test-'))
...
describe('ap billing --help', () => {
  it('lists status and credits subcommands', async () => {
    const result = await ap(['billing', '--help'])
    expect(result.exitCode).toBe(0)
    expect(result.stdout).toContain('status')
    expect(result.stdout).toContain('credits')
  })
})

describe('ap billing status', () => {
  it('exits with auth error (code 2) when no API key is set', async () => {
    const result = await ap(['billing', 'status'])
    expect(result.exitCode).toBe(2)
    expect(result.stderr).toMatch(/No API key found/)
  })
})

describe('ap billing credits', () => {
  it('exits with auth error (code 2) when no API key is set', async () => {
    const result = await ap(['billing', 'credits'])
    expect(result.exitCode).toBe(2)
    expect(result.stderr).toMatch(/No API key found/)
  })
})
